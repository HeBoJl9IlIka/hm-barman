...
namespace LiquidVolumeFX {
    ...
    public partial class LiquidVolume : MonoBehaviour {
        ...
        void FixedUpdate() {
            turbulenceSpeed += UnityEngine.Time.deltaTime * 3f * _speed;
            liqMat.SetFloat(ShaderParams.TurbulenceSpeed, turbulenceSpeed * 4f);
            murkinessSpeed += UnityEngine.Time.deltaTime * 0.05f * (shaderTurb.x + shaderTurb.y);
            liqMat.SetFloat(ShaderParams.MurkinessSpeed, murkinessSpeed);
            bubblesVerticalSpeedAcum += UnityEngine.Time.deltaTime * _bubblesVerticalSpeed;
            if (_detail.isMultiple()) {
        ...
        void SetNewLightTargetPos(int k) {
            ...
            pointLightParams[k].nextTargetTime = UnityEngine.Time.time + Random.value + pointLightParams[k].nextTargetInterval;
        }
        ...
        void UpdateLightEffects() {
            ...
            for (int k = 0; k < pointLightParams.Length; k++) {
                ...
                if (pointLightParams[k].randomTarget && UnityEngine.Time.time > pointLightParams[k].nextTargetTime) {
                    SetNewLightTargetPos(k);
                }
                Vector3 targetPos = pointLightParams[k].targetPos;
                Vector3 accel = pointLightParams[k].accel;
                float speed = pointLightParams[k].moveSpeed * UnityEngine.Time.deltaTime;
            ...
            for (int k = 0; k < 6; k++) {
                ...
                if (_pointLightsEnabled && k < pointLightParams.Length) {
                    ...
                    float intensity = pointLightParams[k].intensityMin + Mathf.PingPong(UnityEngine.Time.time * pointLightParams[k].intensitySpeed, pointLightParams[k].intensityMax - pointLightParams[k].intensityMin);
                ...
                if (range > 0 && multiplier > 0) {
                    ...
                    Color pointLightColor = Color.Lerp(pointLightParams[k].color1, pointLightParams[k].color2, Mathf.PingPong(UnityEngine.Time.time * pointLightParams[k].colorSpeed, 1f));
            ...
            if (Application.isPlaying) {
                for (int j = 0; j < sortedLayersCount; j++) {
                    ...
                    if (_liquidLayers[k].currentBaseLevel != _liquidLayers[k].baseLevel || _liquidLayers[k].currentAmount != _liquidLayers[k].mixedAmount || _liquidLayers[k].currentMurkiness != _liquidLayers[k].mixedMurkiness || _liquidLayers[k].currentBubblesOpacity != _liquidLayers[k].mixedBubblesOpacity || _liquidLayers[k].currentColor != _liquidLayers[k].mixedColor || _liquidLayers[k].currentColor2 != _liquidLayers[k].mixedColor2) {
                        float t = UnityEngine.Time.deltaTime * _layersAdjustmentSpeed * _liquidLayers[k].adjustmentSpeed;
                        if (t > 1f || immediate) {
                            t = 1f;
                        }
...
#if UNITY_EDITOR
            if (UnityEngine.Time.frameCount != fpRenderTextureCheckFrame) {
                fpRenderTextureCheckFrame = UnityEngine.Time.frameCount;
